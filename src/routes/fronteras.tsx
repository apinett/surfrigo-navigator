...
export const Route = createFileRoute("/fronteras")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Ventanas de atención, corte documental y probabilidad de cruce por paso fronterizo con ETA recalculado." },
      { property: "og:title", content: "Fronteras y ETA | Surfrigo Control Tower" },
      { property: "og:description", content: "Cierres de frontera, corte de papeles y recálculo de ETA condicionado a la reapertura." },
    ],
  }),
          ...
          return (
            <section key={border.id} className="panel p-4">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Corte documental {border.paperworkCutoff} · trámite promedio {border.avgProcessMinutes} min
              </p>
                    ...
                    <li key={ev.id} className="rounded-md border border-border bg-surface-strong/60 p-3">
                      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
                        <p className="min-w-0 truncate text-sm">
                          <span className="font-mono font-semibold">{unit?.code}</span> desde {locationName(ev.departedFromId)}
                        </p>
                        <span className={`tabular shrink-0 text-xs font-semibold ${pct < 50 ? "text-st-riesgo" : "text-st-disponible"}`}>
                          {pct}%
                        </span>
                      </div>
                      ...
                      <p className="mt-2 text-xs text-muted-foreground">
                        ...
                        {ev.recalculatedEtaAt && ` · si no cruza: ${fmtStamp(ev.recalculatedEtaAt)}`}
                      </p>
                    </li>
                  );
                })}
